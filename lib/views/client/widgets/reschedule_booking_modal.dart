...
/// Modal bottom sheet that lets a client reschedule an upcoming booking.
///
/// Shows a horizontal 14-day day picker and hourly time slots filtered by the
/// provider's `working_hours`. On confirm, the booking's `scheduled_at` is
/// updated in Supabase and its status is reset to `pending` so the provider
/// has to re-confirm the new slot (the provider is notified of the request).
///
/// Usage:
/// ```dart
...
/// );
/// ```
/// Returns the new [DateTime] when confirmed, or null if dismissed.
class RescheduleBookingModal extends StatefulWidget {
  ...
  /// Convenience method that shows the modal and returns the newly chosen
  /// [DateTime], or null when the sheet is dismissed.
  static Future<DateTime?> show(
    ...
    return showModalBottomSheet<DateTime?>(
      ...
      builder: (_) => RescheduleBookingModal(
        booking: booking,
        provider: provider,
      ),
    );
...
class _RescheduleBookingModalState extends State<RescheduleBookingModal> {
  ...
  // ── Schedule generation (mirrors the booking slot picker) ─────────────
  ...
  void initState() {
    ...
    final current = widget.booking.scheduledAt.toLocal();
  ...
  Future<void> _confirm() async {
    ...
    try {
      // Updates scheduled_at and resets the status to "pending" so the
      // provider has to re-confirm the new slot.
            ...
            child: Column(
              ...
              children: [
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color:
                                active ? const Color(0xFFF4665C) : Colors.white,
                            border: Border.all(
                              color: active
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: const Color(0x14FFB93F),
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: Row(
                    children: [
                      const Icon(Icons.info_outline,
                          size: 16, color: Color(0xFFB07514)),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          'Your booking goes back to "Pending" until the '
                          'provider confirms the new time.\n'
                          'Votre rendez-vous redeviendra "En attente" jusqu\'à '
                ...
                SizedBox(
                  ...
                  child: FilledButton(
                    onPressed: _saving || _selectedTime == null || !_isOpen
                        ? null
                        : _confirm,
