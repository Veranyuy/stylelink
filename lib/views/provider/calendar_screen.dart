...
class _CalendarScreenState extends State<CalendarScreen> {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        return Padding(
          ...
          // Tapping a booking opens the detail sheet with status-aware
          // actions; the realtime stream re-renders the list after any
          // action completes.
          //
          // GestureDetector (not InkWell) with HitTestBehavior.opaque:
          // InkWell's deferToChild hit-testing loses taps at the Card's
          // material boundary/padding edges, and its ink splash paints on
          // the wrong (Scaffold-level) Material. Opaque guarantees the tap
          // registers across the FULL card surface.
          child: GestureDetector(
            behavior: HitTestBehavior.opaque,
            onTap: () {
              debugPrint('Booking card tapped: ${booking.id}');
              _openDetailSheet(booking);
            },
  ...
  /// Resolves client + service names, then opens the detail sheet.
  Future<void> _openDetailSheet(Booking booking) async {
    var clientName = 'Client';
    var serviceName = 'Service';
    try {
      final profiles = await supabase.fetchProfilesByIds([booking.clientId]);
      if (profiles.isNotEmpty) {
        clientName = profiles.first.fullName ?? 'Client';
      }
      if (booking.serviceIds.isNotEmpty) {
        final services = await supabase.fetchServicesByIds(booking.serviceIds);
        if (services.isNotEmpty) {
          serviceName = services.map((s) => s.name).join(' + ');
        }
      }
    } catch (_) {
      // Fall back to placeholders — the sheet is still fully functional.
    }
    if (!mounted) return;
